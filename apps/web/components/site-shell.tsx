...
export async function Footer() {
  ...
  return (
    <footer className="footer">
      ...
      <div className="shell footer-bottom"><span>© {new Date().getFullYear()} YASAWI STARTUP</span><div className="footer-legal"><Link href="/privacy">{copy.nav.privacy}</Link><Link href="/consent">{copy.nav.consent}</Link><Link href="/terms">{copy.nav.terms}</Link></div><span>{copy.footer.slogan} <Globe2 size={14} /></span></div>
    </footer>
  );
}
