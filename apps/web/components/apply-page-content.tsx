...
export function ApplyPageContent({ responseDays = 5 }: { responseDays?: number }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { ...Object.fromEntries(new FormData(event.currentTarget).entries()), locale };
  ...
  return <section className="apply-page"><div className="apply-grid" /><div className="shell apply-layout"><div className="apply-copy"><Eyebrow>{copy.eye}</Eyebrow><h1>{copy.title}<br /><span>{copy.outline}</span></h1><p>{copy.description}</p><div className="apply-points">{copy.points.map((point) => <span key={point}><CircleCheck /> {point}</span>)}</div><p className="application-response-note">{responseNote}</p></div><div className="apply-form-card">{status === "success" ? <div className="form-success"><CircleCheck size={56} /><h2>{copy.success}</h2><p>{copy.thanks}</p><Link className="button primary" href="/">{copy.home}</Link></div> : <form onSubmit={submit}><label>{copy.select}<select name="program" value={program} onChange={(event) => setProgram(event.target.value)} required>{programOptions.map((item) => <option value={item.value} key={item.value}>{item.label}</option>)}</select></label><label>{copy.name}<input name="name" placeholder={copy.namePlaceholder} minLength={2} required /></label><div className="form-row"><label>Email<input name="email" type="email" placeholder="you@example.com" required /></label><label>{copy.phone}<input name="phone" type="tel" placeholder="+7 700 000 00 00" required /></label></div><label>{copy.idea}<textarea name="idea" placeholder={copy.ideaPlaceholder} minLength={10} required /></label><label className="consent-field"><input name="consent" type="checkbox" value="true" required /><span>{copy.consent} <Link href="/privacy" target="_blank">{copy.policy}</Link>.</span></label>{status === "error" && <p className="form-error">{copy.error}</p>}<button className="button primary submit" disabled={status === "loading"}>{status === "loading" ? copy.sending : copy.submit}<ArrowUpRight /></button></form>}</div></div></section>;
}
